...
contract Widget {
    ...
    function updateWidget(
        ...
        require(
            ...
            "Can't update a widget that doesn't exist."
        );
        WidgetStruct storage w = widgets[key];
        w.name = name;
        w.delux = delux;
    ...
    function getWidget(bytes32 key)
        public
        view
        returns (
            string memory name,
    ...
    function getWidgetCount() public view returns (uint256 count) {
        return widgetSet.count();
    }

    function getWidgetAtIndex(uint256 index) public view returns (bytes32 key) {
